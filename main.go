...
func main() {
	...
	if err != nil {
		log.Fatal("aws load default config failed")
	}
	...
	mux.HandleFunc("POST /api/thumbnail_upload/{videoID}", cfg.middlewareAuthenticateUpload(cfg.handlerUploadThumbnail))
	mux.HandleFunc("POST /api/video_upload/{videoID}", cfg.middlewareAuthenticateUpload(cfg.handlerUploadVideo))
